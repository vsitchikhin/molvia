 ...
 * page does no work per frame. The line the sentinel crosses is given by the screen, measured
 * once on mount: the bottom edge of a pinned row, or the top of the window where the row takes
 * no room. Either never moves — which is the whole point: a row that shrank on collapse would
 * pull the content up, bring the sentinel back and flip itself open again.
 *
 ...
 */
export function useCollapsed(sentinel: Ref<HTMLElement | null>, line: () => number): Ref<boolean> {
