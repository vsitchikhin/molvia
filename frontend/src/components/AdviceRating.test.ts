...
const render = (rating: string, count: number, scope: 'own' | 'shared' = 'shared') =>
  mount(AdviceRating, {
    props: { rating, count, scope },
...
describe('AdviceRating', () => {
  ...
  it('the word agrees with the number', () => {
  ...
  it('leaves the count out in the own mode: there it is always one (МР-12)', () => {
    // The subtitle above already says «Пока только ваши оценки», and «· 1 оценка» stood at
    // the end of every row on the screen.
    expect(plain(render('4.8', 1, 'own').text())).toBe('4,8 из 5')
  })
})
