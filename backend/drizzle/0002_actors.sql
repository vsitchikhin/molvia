...
-- Правка руками, одна на весь MOL-6. События писались случайным actor_id, пока таблицы
-- владельцев не существовало, и внешний ключ поверх таких строк не встанет. Продуктовой
-- базы нет — первый деплой это MOL-37, — поэтому осиротевшие строки журнала принадлежат
-- только базам разработки и тестов. Безопасно ровно один раз: дальше ключ не даст им
-- появиться.
